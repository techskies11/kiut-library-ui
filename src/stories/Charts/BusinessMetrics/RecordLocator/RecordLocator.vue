        ...
        <div class="w-full min-w-0">
          ...
          >
            <template #cell-date="{ row }">
              <span class="cell-plain font-medium">{{
                moment(String(row.date)).format("MMM DD")
              }}</span>
            </template>
            <template #cell-checkinInit="{ row }">
              <span class="cell-plain text-center">{{
                useNumberFormat(row.checkin_initiated as number)
              }}</span>
            </template>
            ...
            <template #cell-checkinStarted="{ row }">
              <span class="cell-plain text-center">{{
                useNumberFormat(row.record_locator_started_count as number)
              }}</span>
            </template>
            ...
            <template #cell-checkinClosed="{ row }">
              <span class="cell-plain text-center success-value">{{
                formatValueWithPercentage(
            ...
            <template #cell-checkinFailed="{ row }">
              <span class="cell-plain text-center failed-value">{{
                formatValueWithPercentage(
                  row.record_locator_failed_count as number,
                  row.record_locator_started_count as number,
                )
              }}</span>
            </template>
            <template #cell-abandoned="{ row }">
              <span class="cell-plain text-center warning-value">{{
                formatValueWithPercentage(
                  row.record_locator_abandoned_count as number,
                  row.record_locator_started_count as number,
                )
              }}</span>
            </template>
            ...
            <template #cell-failedPayment="{ row }">
              <span class="cell-plain text-center failed-value">{{
                useNumberFormat(
                  (row.record_locator_create_payment_failed_count as number) ??
                    0,
                )
              }}</span>
            </template>
      ...
      <section v-else class="empty-state">
        <div class="empty-state-content">
          ...
          <p class="empty-description">
            No record locator data found for the selected period. Try adjusting
            the date range.
          </p>
        </div>
      </section>
...
const recordLocatorColumns = computed(() =>
  props.isAvianca
    ? [...baseRecordLocatorColumns, ...aviancaExtraColumns]
    : baseRecordLocatorColumns,
);
...
const recordLocatorTableRows = computed((): Record<string, unknown>[] =>
  tableData.value.map((row) => ({
    ...
    record_locator_create_payment_failed_count:
      row.record_locator_create_payment_failed_count,
  })),
);
...
const sankeyData = computed(() => {
  ...
  const recordLocatorStarted =
    recordLocatorData.value.total_record_locator_started;
  const recordLocatorCompleted =
    recordLocatorData.value.total_record_locator_completed;
  const recordLocatorClosed =
    recordLocatorData.value.total_record_locator_closed;
  const recordLocatorFailed =
    recordLocatorData.value.total_record_locator_failed;
  const recordLocatorAbandoned =
    recordLocatorData.value.total_record_locator_abandoned;
  const recordLocatorInitAbandoned =
    recordLocatorData.value.total_record_locator_init_abandoned;
  const preInitAbandonedErrorRaw =
    recordLocatorData.value.total_checkin_pre_init_abandoned_error;
  const preInitAbandonedVoluntaryRaw =
    recordLocatorData.value.total_checkin_pre_init_abandoned_voluntary;
  ...
  const preInitAbandonedVoluntary = hasPreInitAbandonedSplit
    ...
    : 0;
  const initAbandonedErrorRaw =
    recordLocatorData.value.total_record_locator_init_abandoned_error;
  const initAbandonedVoluntaryRaw =
    recordLocatorData.value.total_record_locator_init_abandoned_voluntary;
  const hasInitAbandonedSplit =
    (initAbandonedErrorRaw !== null && initAbandonedErrorRaw !== undefined) ||
    (initAbandonedVoluntaryRaw !== null &&
      initAbandonedVoluntaryRaw !== undefined);
