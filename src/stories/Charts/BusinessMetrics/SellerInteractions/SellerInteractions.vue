...
export interface SellerInteractionsData {
  avg_sell_interactions_to_complete?: number | null
  avg_sell_interactions_by_day?: Record<string, number | null>
}
...
const props = withDefaults(
  defineProps<{
    loading?: boolean
    data?: SellerInteractionsData | null
    theme?: Theme
  ...
  }>(),
  {
    ...
    subtitle: 'Average number of interaction turns taken to complete the flow',
    emptyTitle: 'No seller interaction data',
    emptyDescription:
...
const processChartData = (data: SellerInteractionsData | null): void => {
  const series = buildSellerInteractionsSeries(data?.avg_sell_interactions_by_day)
  ...
  rawValuesByDay.value = series.values
