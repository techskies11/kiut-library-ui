          ...
          >
            <template #cell-departure="{ row }">
              <span class="segment-plain">{{
                formatAirport(row.departure_airport as string)
              }}</span>
            </template>
            ...
            <template #cell-arrival="{ row }">
              <span class="segment-plain">{{
                formatAirport(row.arrival_airport as string)
              }}</span>
            </template>
            <template #cell-trip="{ row }">
              <span class="segment-plain">
                {{
                  isRoundtrip(row as unknown as SegmentData)
                    ? "Roundtrip"
                    : "One way"
                }}
              </span>
            ...
            <template #cell-started="{ row }">
              <span class="percentage-value">{{
                calculatePercentage(
                  row.segment_started_count as number,
      ...
      <section v-else class="empty-state">
        <div class="empty-state-content">
          ...
          <p class="empty-description">
            No flight segment data found for the selected period. Try adjusting
            the date range.
          </p>
        </div>
      </section>
