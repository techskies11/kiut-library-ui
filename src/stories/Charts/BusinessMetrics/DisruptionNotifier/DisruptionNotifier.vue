  ...
  >
    ...
    <Transition name="bm-fade" mode="out-in">
      ...
      <template v-if="hasData">
          ...
          <div class="chart-wrapper">
            ...
            <div v-else class="empty-chart">
              <p class="empty-chart-text">
                No processing data available for visualization
              </p>
            </div>
          </div>
        </section>
        ...
        <div
          class="grid w-full grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-5"
        >
          <CardInfo
            color="#3b82f6"
            title="Total Records"
          ...
          <CardInfo
            ...
            :subvalue="
              topFailure.count > 0
                ? `${useNumberFormat(topFailure.count)} cases`
                : undefined
            "
          />
        </div>
            ...
            >
              ...
              <template #cell-count="{ row }">
                <span class="failure-count">{{
                  useNumberFormat(row.count as number)
                }}</span>
              </template>
              <template #cell-impact="{ row }">
                <div class="impact-bar-container">
                  <div
                    class="impact-bar"
                    :style="{ width: (row.impactPct as number) + '%' }"
                  ></div>
        ...
        <div class="dn-trend-health-block flex flex-col gap-0">
          ...
          >
            ...
            <div
              class="dn-trend-chart-area min-h-[280px] w-full min-w-0 flex-1"
            >
              <LineChart
                :data="trendChartData"
          ...
          <details class="system-health">
            ...
            <div class="system-health-content">
              <div
                class="grid w-full grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4"
              >
                <CardInfo
                  title="Docs Started"
      ...
      <section v-else class="empty-state">
        <div class="empty-state-content">
          ...
          <p class="empty-description">
            No disruption notification data found for the selected period. Try
...
const procTotals = computed(() => {
  ...
  return {
    ...
    totalDqErrors: items.reduce(
      (s, i) =>
        s +
        i.dq_error_phone_not_found +
        i.dq_error_flight_not_found +
        i.dq_error_booking_not_found +
        i.dq_error_other,
      0,
    ),
  };
...
const affected = computed(
  () => docTotals.value.row_count_total || procTotals.value.processing_started,
);
const notNotified = computed(() =>
  Math.max(0, affected.value - procTotals.value.notification_sent),
);
...
const topFailure = computed(() => {
  const reasons = [
    ...
    { reason: "Flight not found", count: procTotals.value.dq_flight },
    {
      ...
      count: procTotals.value.notification_failed,
    },
...
const funnelData = computed(() => {
  ...
  if (invalidRecords > 0)
    links.push({
      ...
      label: fl(invalidRecords, records),
    });
  if (contactable > 0)
    links.push({
  ...
  if (notContactable > 0)
    links.push({
      ...
      label: fl(notContactable, records),
    });
...
const trendChartData = computed(() => {
  ...
  const allDates = [
    ...new Set([
      ...procItems.map((i) => i.date),
      ...docItems.map((i) => i.date),
    ]),
  ].sort();
...
const trendOptions = computed(() => ({
  ...
  scales: {
    ...
    y: {
      ...
      beginAtZero: true,
      max: 100,
      grid: { color: colors.value.gridLines },
      ticks: {
        font: {
          family: "'Inter', ui-sans-serif, system-ui, sans-serif",
          size: 11,
...
.dn-trend-health-block .system-health {
  margin-top: 0;
}
.dn-trend-chart-area {
  padding-top: 10px;
...
.system-health {
  margin-top: 4px;
  border: 1px solid var(--kiut-border-light, rgba(0, 0, 0, 0.06));
  border-radius: 12px;
  overflow: hidden;
}
.system-health-toggle {
  ...
  font-size: 0.8125rem;
  font-weight: 500;
  color: var(--kiut-text-secondary, #94a3b8);
...
.system-health-toggle:hover {
  color: var(--kiut-text-primary, #475569);
}
.system-health-toggle::-webkit-details-marker {
  display: none;
}
.toggle-icon {
  width: 16px;
...
.system-health[open] .toggle-icon {
